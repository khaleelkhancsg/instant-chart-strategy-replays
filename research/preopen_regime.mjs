...
for (const [lbl, a, b, ff] of [
  ...
  console.log(`   ${lbl.padEnd(34)}${String(s.n).padStart(6)}  ${s.pf.toFixed(3)}  ` +
    `${("$" + s.exp.toFixed(2)).padStart(8)}${f6(passOf(t1, T0, T1))}${f6(passOf(t2, T0, T1))}`);
}// The two tests that killed early arming: does the gain sit in one volatility
// bucket, and does it replicate on an instrument the search never touched?
const A0 = replay(510,900).trades;              // ships
const A1 = replay(420,900,{forceFlatAt:495}).trades;  // extended, flat 08:15
let gw=0,gl=0,t0n=0,t1n=0;
const st=(t)=>{let a=0,b=0,tot=0;for(const x of t){tot+=x.pnl;if(x.pnl>0)a+=x.pnl;else b-=x.pnl;}
  return {n:t.length,pf:b?a/b:Infinity,exp:t.length?tot/t.length:0,net:tot};};
console.log("\nC. NET, since $/trade alone is misleading when trade counts differ\n");
const s0=st(A0), s1=st(A1);
console.log("   RTH only        "+s0.n+" trades  $/t $"+s0.exp.toFixed(2)+"  net $"+(s0.net/1000).toFixed(0)+"k");
console.log("   flat 08:15      "+s1.n+" trades  $/t $"+s1.exp.toFixed(2)+"  net $"+(s1.net/1000).toFixed(0)+"k");

console.log("\nD. REGIME - does the gain sit in one ATR bucket?\n");
const ATRAT=new Map(); for(let i=0;i<n;i++) ATRAT.set(tf.ts[i], A[i]);
const tag=(t)=>ATRAT.get(t.entryTime)||0;
const allA=A0.map(tag).filter(v=>v>0).sort((a,b)=>a-b);
const q=[0.2,0.4,0.6,0.8].map(p=>allA[Math.floor(p*allA.length)]);
const qi=(v)=>v<q[0]?0:v<q[1]?1:v<q[2]?2:v<q[3]?3:4;
console.log("   quintile  ATR range        ships $/t   flat0815 $/t    delta       n     gain");
let tot=0; const gains=[];
for(let i2=0;i2<5;i2++){
  const a=st(A0.filter(t=>qi(tag(t))===i2)), b=st(A1.filter(t=>qi(tag(t))===i2));
  const g=(b.exp-a.exp)*b.n; gains.push(g); tot+=g;
  const rng=i2===0?("< "+q[0].toFixed(1)):i2===4?(">= "+q[3].toFixed(1)):(q[i2-1].toFixed(1)+"-"+q[i2].toFixed(1));
  console.log("   "+String(i2+1).padStart(8)+"  "+rng.padEnd(14)+("$"+a.exp.toFixed(2)).padStart(10)+
    ("$"+b.exp.toFixed(2)).padStart(15)+((b.exp-a.exp>=0?"+":"")+"$"+(b.exp-a.exp).toFixed(2)).padStart(9)+
    String(b.n).padStart(8)+("$"+(g/1000).toFixed(1)+"k").padStart(9));
}
console.log("   total $"+(tot/1000).toFixed(1)+"k, single best quintile "+
  (100*Math.max(...gains)/tot).toFixed(0)+"% of it");
