...
for (let cut = T0 + 730 * 86400000; cut + HALF <= T1; cut += HALF) {
  ...
  for (const k of KGRID) {
    ...
    const s = stat(tr);
    if (s.exp > bestScore) { bestScore = s.exp; bestK = k; }
  }
...
// ── 2. MECHANISM ────────────────────────────────────────────────────
console.log("\n2. MECHANISM — is it the arming SET, or the cheaper FILL?\n");
console.log("   Pushing the trigger out by k restores roughly the original entry");
console.log("   price while keeping the new arming set.\n");
console.log("   variant                                   fills    pass     pf    $/trade");
for (const [lbl, k, trig] of [
  ["k=0.00, trigger 0.15 (ships)", 0, 0.15],
  ["k=0.40, trigger 0.15 (proposed)", 0.40, 0.15],
  ["k=0.40, trigger 0.55 (fill restored)", 0.40, 0.55],
  ["k=0.00, trigger 0.55 (trigger only)", 0, 0.55],
]) {
  const b = replay(sigOf(k), trig), s = stat(b);
  console.log(`   ${lbl.padEnd(40)}${String(s.n).padStart(6)}  ${passOf(b, T0, T1).toFixed(1).padStart(6)}  ` +
    `${s.pf.toFixed(3)}  ${("$" + s.exp.toFixed(2)).padStart(8)}`);
}

// ── 3. REGIME ───────────────────────────────────────────────────────
console.log("\n3. REGIME — by ATR quintile at entry. One-regime effects are bets.\n");
{
  const all = BOOKS.get(0).map(t => t.atr).sort((a, b) => a - b);
  const q = [0.2, 0.4, 0.6, 0.8].map(p => all[Math.floor(p * all.length)]);
  const band = (t, i) => (i === 0 ? t.atr < q[0]
    : i === 4 ? t.atr >= q[3] : t.atr >= q[i - 1] && t.atr < q[i]);
  console.log("   quintile   ATR range        k=0 $/trade   k=0.40 $/trade   delta    n");
  for (let i = 0; i < 5; i++) {
    const a = stat(BOOKS.get(0).filter(t => band(t, i)));
    const b = stat(BOOKS.get(0.40).filter(t => band(t, i)));
    const rng = i === 0 ? `< ${q[0].toFixed(1)}` : i === 4 ? `>= ${q[3].toFixed(1)}`
      : `${q[i - 1].toFixed(1)}-${q[i].toFixed(1)}`;
    console.log(`   ${String(i + 1).padStart(8)}   ${rng.padEnd(14)}${("$" + a.exp.toFixed(2)).padStart(11)}   ` +
      `${("$" + b.exp.toFixed(2)).padStart(13)}  ${((b.exp - a.exp >= 0 ? "+" : "") + "$" + (b.exp - a.exp).toFixed(2)).padStart(8)}  ${String(b.n).padStart(5)}`);
  }
}

// ── 4. ESTIMATOR ────────────────────────────────────────────────────
console.log("\n4. ESTIMATOR — resample TRADES, not day-blocks, in case the day");
console.log("   bootstrap is what produced the result\n");
{
  const boot = (t, seed) => {
    const rnd = mul(seed); const N = t.length; let wins = 0;
    for (let d = 0; d < 4000; d++) {
      let s = 0;
      for (let j = 0; j < 400; j++) s += t[Math.floor(rnd() * N)].pnl;
      if (s > 0) wins++;
    }
    return 100 * wins / 4000;
  };
  console.log("   variant     $/trade   P(400-trade block profitable)");
  for (const k of [0, 0.40, 0.60]) {
    const s = stat(BOOKS.get(k));
    console.log(`   k=${k.toFixed(2)}    ${("$" + s.exp.toFixed(2)).padStart(8)}   ${boot(BOOKS.get(k), 555).toFixed(1)}%`);
  }
}
