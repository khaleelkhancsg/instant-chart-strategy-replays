...
function replay(startCt, endCt, { costMult = 1, forceFlatAt = -1 } = {}) {
  ...
  const close_ = (rawExit, i, exact) => {
    ...
    out.push({ tday: TD[i], entryTime: entTime, entCt, pnl: net, heldOpen });
    dayReal += net;
...
console.log("\nA. FLAT-BEFORE-OPEN, swept. Plateau or knife-edge?\n");
console.log("   variant                     trades    pf   $/trade      1x     2x   early    late    2026");
const CANDS = [[510,900,-1,"RTH only (ships)"],[420,900,-1,"extended, carry in"],
  [420,900,480,"extended, flat 08:00"],[420,900,495,"extended, flat 08:15"],
  [420,900,509,"extended, flat 08:29"],[450,900,509,"07:30 start, flat 08:29"],
  [360,900,509,"06:00 start, flat 08:29"],[300,900,509,"05:00 start, flat 08:29"]];
const BOOKS = CANDS.map(([a,b,ff])=>replay(a,b,{forceFlatAt:ff}).trades);
CANDS.forEach(([a,b,ff,lbl],i)=>{
  const t1=BOOKS[i], t2=replay(a,b,{costMult:2,forceFlatAt:ff}).trades;
  const st=stat(t1);
  console.log("   "+lbl.padEnd(26)+String(st.n).padStart(6)+"  "+st.pf.toFixed(3)+"  "+
    ("$"+st.exp.toFixed(2)).padStart(8)+f6(passOf(t1,T0,T1))+f6(passOf(t2,T0,T1))+
    f6(passOf(t1,T0,MID))+f6(passOf(t1,MID,T1))+f6(passOf(t1,Y26,T1)));
});
{ let ss=0; const idx=[2,3,4];
  for(let k=1;k<idx.length;k++) ss+=Math.abs(passOf(BOOKS[idx[k]],T0,T1)-passOf(BOOKS[idx[k-1]],T0,T1));
  console.log("\n   mean |step| across flat times: +-"+(ss/(idx.length-1)).toFixed(2)+"pp"); }

console.log("\nB. WALK-FORWARD - variant chosen on the past only, by pass rate\n");
console.log("   test period               chosen                 ships $/t  chosen $/t     delta");
const SHIP=0; const wfT=[],shT=[],picks=[];
for(let cut2=T0+730*86400000; cut2+HALF<=T1; cut2+=HALF){
  let bi=SHIP,best=-Infinity;
  BOOKS.forEach((bk,i)=>{ const tr=bk.filter(t=>t.entryTime<cut2); if(tr.length<150)return;
    const sc=passOf(bk,T0,cut2,777); if(Number.isFinite(sc)&&sc>best){best=sc;bi=i;} });
  const lo=cut2, hi=Math.min(cut2+HALF,T1);
  const te=BOOKS[bi].filter(t=>t.entryTime>=lo&&t.entryTime<hi);
  const t0=BOOKS[SHIP].filter(t=>t.entryTime>=lo&&t.entryTime<hi);
  if(te.length<20||t0.length<20) continue;
  picks.push(bi); wfT.push(...te); shT.push(...t0);
  const a2=stat(te),b2=stat(t0);
  console.log("   "+new Date(lo).toISOString().slice(0,10)+".."+new Date(hi).toISOString().slice(0,10)+
    "  "+CANDS[bi][3].padEnd(22)+("$"+b2.exp.toFixed(2)).padStart(10)+("$"+a2.exp.toFixed(2)).padStart(11)+
    ((a2.exp-b2.exp>=0?"+":"")+"$"+(a2.exp-b2.exp).toFixed(2)).padStart(10));
}
{ const w=stat(wfT),sp=stat(shT),lo2=T0+730*86400000;
  const nFlat=picks.filter(p=>CANDS[p][2]>0).length;
  console.log("\n   a flat-before-open variant chosen "+nFlat+"/"+picks.length);
  console.log("   walk-forward   pf "+w.pf.toFixed(3)+"  $/t $"+w.exp.toFixed(2)+
    "  PASS "+passOf(wfT,lo2,T1).toFixed(1)+"%");
  console.log("   always shipped pf "+sp.pf.toFixed(3)+"  $/t $"+sp.exp.toFixed(2)+
    "  PASS "+passOf(shT,lo2,T1).toFixed(1)+"%"); }
