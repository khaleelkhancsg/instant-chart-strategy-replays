...
// PUSH IT UNTIL IT BREAKS. The k sweep was monotone up to 0.40, which is the
// shape that should make anyone nervous: if arming earlier keeps helping without
// limit, the Donchian channel is not doing any work and the "breakout" framing is
// wrong. A real effect has to turn over somewhere. Extended far past anything
// sensible to find that point -- k = 1.0 arms a full ATR before the channel, and
// k = 2.0 is effectively "arm on any bar the gate allows".
const CFG = [
  ["k=0.00 (ships)", 0.00, 0.15],
  ["k=0.25", 0.25, 0.15],
  ["k=0.40", 0.40, 0.15],
  ["k=0.60", 0.60, 0.15],
  ["k=0.80", 0.80, 0.15],
