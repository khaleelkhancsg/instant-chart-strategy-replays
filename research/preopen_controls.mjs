...
const BIN = process.env.BIN || "data/mnq_1m.bin";
const PV_ = Number(process.env.PV || 2);
...
const sigAll = applyFilters(raw, ctx, { ...NO_FILTER, startCt: 0, endCt: 1440, effMin: EFF });
const ALLDAYS = (() => {
  const m = new Map();
...
const LBL = process.env.LABEL || "MNQ";
console.log("");
console.log(LBL + "   eff=" + EFF + "  pv=" + PV_);
console.log("   variant                    trades    pf   $/trade      1x     2x");
const SET = [["RTH only 08:30-15:00",510,900,-1],
             ["extended, flat 08:15",420,900,495],
             ["07:30 start, flat 08:29",450,900,509]];
for (const [lbl,a,b,ff] of SET) {
  const t1 = replay(a,b,{forceFlatAt:ff}).trades;
  const t2 = replay(a,b,{costMult:2,forceFlatAt:ff}).trades;
  const st1 = stat(t1);
  console.log("   " + lbl.padEnd(26) + String(st1.n).padStart(6) + "  " + st1.pf.toFixed(3) +
    "  " + ("$"+st1.exp.toFixed(2)).padStart(8) + f6b(passOf(t1,T0,T1)) + f6b(passOf(t2,T0,T1)));
}
