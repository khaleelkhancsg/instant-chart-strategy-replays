...
const CFG = [
  ["ORIGINAL as it would have traded", 2, true],
  ["ORIGINAL as documented", 2, false],
  ["interim: 1 + 7", 1, false],
  ["NEW: 0 + 8 stop entry", 0, false],
  ["reference: 8 at market", TOTAL, false],
];
const books = CFG.map(([, f, sb]) => replay(f, sb));
...
CFG.forEach(([lbl], i) => {
  let row = "  " + lbl.padEnd(34) + String(books[i].trades.length).padStart(6);
...
CFG.forEach(([lbl], i) => {
  ...
  console.log(`  ${lbl.padEnd(34)}${String(t.length).padStart(6)}  ` +
...
CFG.forEach(([lbl], i) => {
  ...
  console.log(`  ${lbl.padEnd(34)}${(sw.summary.passRate.toFixed(2) + "%").padStart(10)}   ` +
    `${sw.summary.n}`);
});
